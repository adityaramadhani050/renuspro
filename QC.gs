...
function setQCAssignment(noWO, userIds, assignedBy) {
  ...
  try {
    ...
    if (typeof _notifAssignEngineer === 'function') _notifAssignEngineer('QC', noWO, newIds, assignedBy);
...
function getAvailableWOForQC() {
  try {
    ...
    // WO Material-only tidak memerlukan QC → dikecualikan dari daftar.
...
function addQCProject(noWO, userIds, addedBy) {
  ...
  try {
    ...
    if (typeof _woIsMaterialOnly === 'function' && _woIsMaterialOnly(noWO)) {
      return { success: false, message: 'WO jenis Material saja tidak memerlukan QC.' };
    }
    lock.waitLock(15000);
    var ss = getSpreadsheet();
